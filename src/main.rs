...
fn medusa_float_to_string(context: &mut CompilerContext) {
    ...
    let loop_index_3 = context.label_index;
    context.label_index += 1;
...
#[cfg(test)]
mod tests {
    use crate::compile_from_text;

    // Retrieve all of the output from a compiled medusa program
    fn compile_and_get_output(source_text: String) -> String {
        compile_from_text(source_text, "test".to_string()).unwrap();
    ...
    fn compile_and_get_stripped_output(source_text: String) -> String {
    ...
    fn load_test_from_directory(test_name: &str) -> String {
    ...
    fn test_hello_world() {
        assert_eq!(
            compile_and_get_stripped_output(load_test_from_directory("hello_world.med")),
            "Hello, world!"
        );
    }
}
